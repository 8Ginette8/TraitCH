...
# 
# Description: Plot subfigures for Fig. 1 & 4
#
# ###################################################################################
...
# Raw trait files
list.traits = list.files("data/raw_traits/",full.names=TRUE)
...
# First taxo level
taxa.L1 = gsub("data/raw_traits/s2z_raw_traits_|.txt","",list.traits)
...
save_image(outter.plot,scale=4,file="plot/plot_nested_outter.png",width=1100,height=1200)

# Legend
plot(1,1,xlim=c(0,100),ylim=c(0,100),type="n")
legend(0,100,names(group.colors),
      col=group.colors,bty="n",cex=1.8,lwd=30,text.font=2)
...
taxa = str_to_title(gsub("data/raw_traits/s2z_raw_traits_|.txt","",list.traits))
taxa[6] = "EPT"
...
eval.files = list.files("data/missRanger_evaluations",full.names=TRUE)
...
names(box.val) = c("Amphibia","Aves","Apocrita","Bryobiotina","Coleoptera","EPT",
  ...
  "Orthoptera","Tracheophyta","Reptilia","Arachnida")[seq(box.val)]
...
## NOTE: Need to run full TCH1*.R & TCH2*.R to obtain all imputation outputs.
## Not done to avoid a too heavy repository on gitHub.
